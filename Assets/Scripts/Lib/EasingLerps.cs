...
class EasingLerps
{
    ...
    static float EaseOutElastic(float time, float a, float b, float lerpTotal = 1.0f)
    {
        ...
        float i = b;
        float s = p / 4;
        return (i * (float)Math.Pow(2, -10 * time) * (float)Math.Sin((time * lerpTotal - s) * (2 * (float)Math.PI) / p) + b + a);
    }
    ...
    static float EaseInOutElastic(float time, float a, float b, float lerpTotal = 1.0f)
    {
        ...
        if (time < 1)
        {
            return -.5f * (i * (float)Math.Pow(2, 10 * (time -= 1)) * (float)Math.Sin((time * lerpTotal - s) * (2 * (float)Math.PI) / p)) + a;
        }
        return i * (float)Math.Pow(2, -10 * (time -= 1)) * (float)Math.Sin((time * lerpTotal - s) * (2 * (float)Math.PI) / p) * .5f + b + a;
    }
    #endregion
